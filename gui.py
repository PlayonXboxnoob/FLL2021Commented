...
import threading
import tkinter as tk # Library Interface
...
####################### DETECTING KEY PRESSES AND HOW LONG THEY ARE HELD###########################
...
def Launch(): # Defining Launch Button : where it is, and what it does #
    global launched
    global launchText
    global launchBtn
    if False == launched:
...
def destroy(): # Stopping
    global panel
    drone.streamoff()
    panel.destroy()


def createImage(): # Taking in video and putting it GUI
...
def videoLoop(mirror=False): # Keeping the video running
    global panel
    global forward_back
    global up_down
    global left_right
    ...
    while True:
        drone.send_rc_control(0, forward_back, up_down, left_right) # controlling drone movement
        img = createImage()
...
################################ PLACING BUTTONS ON THE SCREEN AND WHAT THEY ARE CONNECTED TO ###########################
...
######### DEFINING GUI WINDOW PARAMETERS ##################
...
root.mainloop() # Loop so the GUI keeps taking input
